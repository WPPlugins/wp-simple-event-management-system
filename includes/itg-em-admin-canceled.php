    ...
    <p><span class="description">The search result will display all approved and unapproved candidates</span></p>
        ...
        if($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            <table class="widefat">
                ...
                <tbody>
                    <?php
                    $attendee_search_results = ((0 == $_POST['search_op'])? itg_em_admin_attendee_search_uid($_POST['search_op'], 0, 0) : itg_em_admin_attendee_search_email($_POST['search_op'], 0, 0));
            ...
            $pagination_count = $wpdb->get_var($wpdb->prepare("SELECT COUNT(id) FROM $itg_em_db_table_name[reg_table] WHERE user_apply=%d AND pay_status=%d", 0, 0));
            if($pagination_count > 0) {
                ...
                $p->limit(10);
                $p->target("admin.php?page=itg_event_admin_cancelled");
                ...
                $attendees = itg_em_admin_attendee_cancel_list($list_start);
                if($attendees) {
                    ?>
                    <table class="widefat">
                        <thead>
                ...
                ?>
                <div class="error fade">Sorry Admin! No one has cancelled yet! May be later.</div>
                <?php
            }
        }
        ?>
